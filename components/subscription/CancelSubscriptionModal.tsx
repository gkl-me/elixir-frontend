...
import { Ban, CheckCircle, Loader2, AlertTriangle, Clock, ZapOff, Info } from "lucide-react";
...
export function CancelSubscriptionModal({
  ...
  return (
    <CustomModal
      isOpen={isOpen}
      onClose={onClose}
      title={isReactivationView ? "Re-activate Subscription" : "Cancel Subscription"}
      description={
        isReactivationView
    ...
    >
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3.5 rounded-xl border border-purple/20 bg-navy/80 p-3.5">
          ...
          <span
            className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold capitalize ${isCanceled
        ...
        {!isReactivationView && (
          <div className="space-y-3">
            <p className="text-xs font-semibold text-gray-300">Select Cancellation Strategy:</p>

            {/* Option A: End of Period */}
            <div
              onClick={() => setCancelMode("period_end")}
              className={`cursor-pointer rounded-xl border p-3.5 transition-all duration-200 ${cancelMode === "period_end"
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-semibold text-white">Cancel at End of Billing Period</p>
                    <span className="rounded-full bg-emerald-500/10 px-2 py-0.5 text-[10px] font-medium text-emerald-400">
                      Recommended
                    </span>
                  </div>
                  <p className="text-xs text-gray-400 leading-relaxed">
                    Customer keeps paid features until end of period. <strong className="text-white">Can be reactivated anytime</strong> before the cycle ends.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div
              ...
              className={`cursor-pointer rounded-xl border p-3.5 transition-all duration-200 ${cancelMode === "immediate"
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-1">
                  <p className="text-sm font-semibold text-white">Immediate Cancellation</p>
                  <p className="text-xs text-gray-400 leading-relaxed">
                    Revokes features immediately. {!isFree ? <strong className="text-red-400">Paid plans cannot be reactivated directly in Stripe</strong> : <span>Free plans can be reactivated anytime.</span>}
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-start gap-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-3.5 text-emerald-200">
                ...
                <div className="space-y-1 text-xs">
                  <p className="font-semibold text-emerald-400">
                    {isPendingCancel ? "Un-cancel & Resume Auto-Renewal" : "Restore Free Subscription"}
                  </p>
                  <p className="leading-relaxed text-emerald-200/90">
                    {isPendingCancel ? (
                      <>
                        Re-activating <strong className="text-white">{subscription.workspaceName}</strong> will remove the pending cancellation and resume normal recurring billing on the renewal date.
                      </>
                    ) : (
                      <>
                        Re-activating <strong className="text-white">{subscription.workspaceName}</strong> will restore access to the Free tier features immediately.
                      </>
                    )}
                  </p>
                </div>
              ...
              <div className="flex items-start gap-3 rounded-xl border border-red-500/40 bg-red-500/10 p-3.5 text-red-200">
                ...
                <div className="space-y-1 text-xs">
                  ...
                  <p className="leading-relaxed text-red-200/90">
                    This paid <strong className="text-white">{subscription.planName}</strong> plan was canceled immediately. Stripe strictly locks canceled paid subscriptions. To restore access, the workspace customer must start a <strong className="text-white">new Checkout session</strong>.
                  </p>
                </div>
              </div>
            )}
        ...
        <div className="flex gap-2 border-t border-purple/20 pt-3.5">
          {!isReactivationView ? (
            <Button
              ...
              disabled={loading}
              className={`flex-1 gap-2 font-semibold text-white shadow-md transition-all duration-200 disabled:opacity-50 ${cancelMode === "immediate"
            ...
            >
              ...
              {cancelMode === "immediate" ? "Cancel Immediately" : "Cancel at Period End"}
            </Button>
          ) : (
            <Button
              onClick={handleConfirm}
              disabled={loading || !canReactivate}
              className="flex-1 gap-2 font-semibold text-white shadow-md transition-all duration-200 disabled:opacity-40 bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-500 hover:to-emerald-600 shadow-emerald-900/20"
            >
              {loading ? (
                <Loader2 className="h-4 w-4 animate-spin" />
              ) : (
