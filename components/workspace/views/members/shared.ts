...
export const ROLE_BADGE: Record<string, { color: string; bg: string; icon: React.ElementType }> = {
...
export const STATUS_BADGE: Record<string, { color: string; bg: string; icon: React.ElementType; label: string }> = {
...
export const allRoles = () => [
  ...
  ...demoWorkspace.customRoles.map(r => ({
    id: r.id, label: r.name, desc: `Custom · ${r.permissions.length} permissions`,
  })),
];
